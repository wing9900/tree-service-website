...
const Blog = () => {
  const blogPosts = [{
    ...
    content: [{
      ...
      type: "list",
      items: ["**Removing Dead and Diseased Branches:** We expertly remove dead, dying, or diseased limbs, preventing the spread of problems that thrive in our moist environment. This also reduces the risk of branch failure during strong winds.", "**Improving Air Circulation:** Thinning the canopy allows for better airflow, which helps to dry out foliage and reduce the incidence of fungal diseases common in our area.", "**Structural Pruning for Young Trees:** Establishing a strong structure early in a tree's life is crucial for its long-term health and resilience against storms. Our arborists understand the specific needs of Houston's diverse tree species."]
    }, {
      type: "heading",
      ...
      type: "list",
      items: ["**Hazard Reduction Pruning:** We identify and remove branches that are weakly attached, overextended, or pose a risk to your home, power lines, or other structures. This proactive approach can significantly reduce storm damage.", "**Crown Reduction for Wind Resistance:** Properly reducing the crown of a tree can decrease wind resistance, making it less likely to be uprooted or suffer major branch damage during a hurricane or tropical storm. Important Note: This should only be done by qualified arborists to avoid harming the tree.", "**Emergency Tree Removal:** After a storm, we provide prompt and safe emergency tree removal services to clear fallen trees and debris, helping you recover quickly."]
    }, {
      type: "heading",
    ...
    content: [{
      ...
      type: "list",
      items: ["**Premature Leaf Drop:** Are leaves falling in spring or summer?", "**Discolored Leaves:** Yellowing (chlorosis), browning, or spotted leaves can signal nutrient deficiencies, pest issues, or fungal diseases common in Houston's humidity.", "**Thinning Canopy:** If you can see much more daylight through the tree's canopy than you used to, it's a sign of significant stress or dieback."]
    }, {
      type: "heading",
      ...
      type: "list",
      items: ["**Deep Cracks or Splits:** These can compromise the tree's structural integrity.", "**Peeling Bark or Cankers:** Sunken or discolored areas of bark (cankers) can indicate a deep-seated infection.", "**Fungus or Mushrooms:** The appearance of mushrooms or other fungal bodies, especially near the base of the tree, is a strong indicator of internal rot and decay."]
    }, {
      type: "heading",
      ...
      type: "list",
      items: ["**Small Holes in the Bark:** This can be a sign of wood-boring insects.", "**Sawdust-like Material (Frass):** This material at the base of the tree or in bark crevices is a byproduct of insect activity.", "**Visible Infestations:** Large numbers of insects on the leaves or bark."]
    }, {
      type: "paragraph",
  ...
  const renderContent = (content: any[]) => {
    return content.map((block, index) => {
      switch (block.type) {
        case 'heading':
          return <h2 key={index} className="text-xl font-semibold text-foreground mt-8 mb-4 first:mt-0">
              {block.text}
            </h2>;
        case 'paragraph':
          return <p key={index} className="text-muted-foreground leading-relaxed mb-4" dangerouslySetInnerHTML={{
            __html: block.text.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
          }} />;
        case 'list':
          return <ul key={index} className="list-disc list-inside space-y-2 mb-6 ml-4">
              {block.items.map((item: string, itemIndex: number) => <li key={itemIndex} className="text-muted-foreground leading-relaxed" dangerouslySetInnerHTML={{
